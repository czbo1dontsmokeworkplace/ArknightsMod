...
namespace ArknightsMod.Content.Tiles.Infrastructure.ReceptionRoom
{
	public class SafeBoxTile : ModTile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			TileObjectData.newTile.Width = BoxWidthTiles;
			TileObjectData.newTile.Height = BoxHeightTiles;
			int originX = BoxWidthTiles / 2;
			TileObjectData.newTile.Origin = new Point16(originX, BoxHeightTiles - 1);
			...
			TileObjectData.newAlternate.Origin = new Point16(originX, BoxHeightTiles - 1);
		...
		{
			const int step = 16 + 1;
			int styleStride = BoxWidthTiles * step;
		...
		public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
		{
			...
			int srcX = (flip ? (BoxWidthTiles - 1 - localX) : localX) * step;
