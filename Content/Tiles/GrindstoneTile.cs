...
namespace ArknightsMod.Content.Tiles
{
	...
	{
		public static LocalizedText GrindstonePassMessage { get; private set; }
		...
		protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration) {
			...
			for (int k = 0; k < (int)(Main.maxTilesX * Main.maxTilesY * 6E-04); k++) {
				...
				if (tile.HasTile && tile.TileType == TileID.Dirt) {
					WorldGen.TileRunner(x, y, WorldGen.genRand.Next(4, 6), WorldGen.genRand.Next(5, 8), ModContent.TileType<GrindstoneTile>());
				}
			}
		}
	}
