...
namespace ArknightsMod.Content.Tiles
{
	public class MnOre : ModTile
	{
		public override void SetStaticDefaults() {
			...
			AddMapEntry(new Color(79, 57, 122), name); // Adds an entry to the minimap for this tile with the given color and display name. This should be called in SetDefaults. 
	...
	public class MnOreSystem : ModSystem
	{
		public static LocalizedText MnOrePassMessage { get; private set; }

		public override void SetStaticDefaults() {
			MnOrePassMessage = Language.GetOrRegister(Mod.GetLocalizationKey($"WorldGen.{nameof(MnOrePassMessage)}"));
		}

<<<<<<< Updated upstream:Content/Tiles/MnOre.cs
...
=======
		public override void ModifyWorldGenTasks(List<GenPass> tasks) {
			...
			// The first step is an Ore. Most vanilla ores are generated in a step called "Shinies", so for maximum compatibility, we will also do this.
			// First, we find out which step "Shinies" is.
>>>>>>> Stashed changes:Content/Tiles/ManganeseOre.cs
			int ShiniesIndex = tasks.FindIndex(genpass => genpass.Name.Equals("Shinies"));

			if (ShiniesIndex != -1) {
				// Next, we insert our pass directly after the original "Shinies" pass.
				// ExampleOrePass is a class seen bellow
				tasks.Insert(ShiniesIndex + 1, new MnOrePass("Manganese Ore", 237.4298f));
			}
		}
	}

	public class MnOrePass : GenPass
	{
		public MnOrePass(string name, float loadWeight) : base(name, loadWeight) {
		}
		...
		protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration) {
			...
			progress.Message = "Manganese Ore";
