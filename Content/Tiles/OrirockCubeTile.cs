...
{
	public class OrirockCubeTile : ModTile
	{
		public override void SetStaticDefaults() {
			...
			AddMapEntry(new Color(198, 181, 136), name); // Adds an entry to the minimap for this tile with the given color and display name. This should be called in SetDefaults. 
	...
	public class OrirockCubeSystem : ModSystem
	{
		public static LocalizedText OrirockCubePassMessage { get; private set; }
		...
		public override void ModifyWorldGenTasks(List<GenPass> tasks) {
			...
			if (ShiniesIndex != -1) {
				...
				tasks.Insert(ShiniesIndex + 1, new OrirockCubePass("Orirock Cubes", 237.4298f));
			}
		}
	}

	public class OrirockCubePass : GenPass
	{
		public OrirockCubePass(string name, float loadWeight) : base(name, loadWeight) {
		}
		...
		protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration) {
			...
			progress.Message = "Orirock Cubes";
			...
			for (int k = 0; k < (int)(Main.maxTilesX * Main.maxTilesY * 6E-04); k++) {
				...
				if (tile.HasTile && tile.TileType == TileID.Dirt) {
					WorldGen.TileRunner(x, y, WorldGen.genRand.Next(5, 7), WorldGen.genRand.Next(6, 9), ModContent.TileType<OrirockCubeTile>());
				}
			}
		}
	}
