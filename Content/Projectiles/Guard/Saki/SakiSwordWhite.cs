...
{
    public class SakiSwordWhite : ModProjectile
    {
        //起始位置
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            Projectile.NewProjectile
				...
				ProjectileType<SakiSlashHit2>(), 0, 0,Projectile.owner);
        ...
        public override void AI()
        {
            ...
            Timer++;
            float cosAngle = (float)Math.Cos(startVector.RotatedBy(Rot).ToRotation());
            float sinAngle = (float)Math.Sin(startVector.RotatedBy(Rot).ToRotation());
	...
	public static class SakiSwordHelper
	{
		public static Vector3 ToVector3(this Vector2 vector) => new Vector3(vector.X, vector.Y, 0);
	}
	public class SakiSwordSystem : ModSystem
	{
		public override void Load()
		{
			if (!Main.dedServ)
			{
				PremultiplyTexture(Request<Texture2D>("ArknightsMod/Content/Projectiles/Guard/Saki/Assets/oblvns_skin_04", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value);
				PremultiplyTexture(Request<Texture2D>("ArknightsMod/Content/Projectiles/Guard/Saki/Assets/oblvns_skin_05", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value);
			}
		}
