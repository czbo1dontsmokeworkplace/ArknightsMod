		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
			...
			if (!hasHit)
			{
				...
				for (int i = 0; i < 6; i++) {
					...
					var p = new DefaultParticleNonPre(target.Center + new Vector2(Main.rand.Next(-Projectile.width, Projectile.width), Main.rand.Next(-Projectile.height, Projectile.height)) / 2f,
					newVelocity, 90, Main.rand.NextFloat(0.35f, 0.5f) * 4f, Color.Black, true);
					p.Deformation = new Vector2(0.1f, 0.6f) * Main.rand.NextFloat(1, 2);
					p.Spawn();
				}
				for (int i = 0; i < 6; i++) {
					Vector2 newVelocity = Projectile.velocity.RotatedByRandom(MathHelper.ToRadians(30));
					newVelocity *= Main.rand.NextFloat(0.1f, 0.35f) * 1.5f;
					var p = new DefaultParticle(target.Center + new Vector2(Main.rand.Next(-Projectile.width, Projectile.width), Main.rand.Next(-Projectile.height, Projectile.height)) / 2f,
					newVelocity, 90, Main.rand.NextFloat(0.35f, 0.5f) * 4f, new Color(249, 90, 100), true);
					p.Deformation = new Vector2(0.1f, 0.6f) * Main.rand.NextFloat(1, 2);
					p.Spawn();
				}
