	...
	public class WisdelShotLarge : ModProjectile
	{
		...
		public void HitEffect(Vector2 center) {
			...
			for (int i = 0; i < 12; i++) {
				...
				var p = new DefaultParticleNonPre(center,
			...
			for (int i = 0; i < 12; i++) {
				Vector2 newVelocity = Projectile.velocity.RotatedByRandom(MathHelper.ToRadians(360));
				newVelocity *= Main.rand.NextFloat(0.1f, 0.35f) * 1.5f;
				var p = new DefaultParticle(center,
			...
			for (int i = 0; i < Main.rand.Next(6, 12); i++) {
				...
				if (!Main.rand.NextBool(4)) {
					var p = new DefaultParticle(center + new Vector2(Main.rand.Next(-Projectile.width, Projectile.width), Main.rand.Next(-Projectile.height, Projectile.height)) / 2f,
					...
					var p = new DefaultParticleNonPre(center + new Vector2(Main.rand.Next(-Projectile.width, Projectile.width), Main.rand.Next(-Projectile.height, Projectile.height)) / 2f,
