...
public class Cardi_Sword : ModProjectile
{
	...
	private Texture2D SwordTex { get => TextureAssets.Projectile[ModContent.ProjectileType<Cardi_Sword>()].Value; }
	private float Length { get => MathF.Sqrt(MathF.Pow(SwordTex.Width, 2) + MathF.Pow(SwordTex.Height, 2)); }
	private Vector2 DrawSetoff => new(12f, 0f);
	...
	public void Move() {
		...
		float moveSwordRot = Projectile.rotation;
		...
		if (Main.myPlayer == player.whoAmI) {
			...
			if (!Main.mouseRight &&
				...
				press = true;
				MouseRad = MathF.Atan2((Main.MouseWorld - player.MountedCenter).Y,
					(Main.MouseWorld - player.MountedCenter).X);
	...
	private Vector2 GetSwordTipWorld(float swordRotation) =>
		GetSwordBaseWorld(swordRotation) + new Vector2(Length, 0f).RotatedBy(swordRotation);
	...
	private void DrawSwordBody(float swordRotation, Color color) {
		...
		Vector2[] drawPos = [
			leftDown + halfPos - halfLength * player.direction,
			leftDown + 2 * halfPos,
			leftDown,
			leftDown + halfPos + halfLength * player.direction
		];
		...
		if (vertices.Count > 4)
			Main.graphics.GraphicsDevice.DrawUserPrimitives(PrimitiveType.TriangleStrip, vertices.ToArray(), 0,
				vertices.Count / 3);
	}

	public void Move_Draw(SpriteBatch sb) {
		float setoff = MathHelper.Pi / 2f;
	...
	public void Attack() {
		...
		player.SetCompositeArmBack(true, Player.CompositeArmStretchAmount.Full, armRot);
		handPos = player.GetBackHandPosition(Player.CompositeArmStretchAmount.Full, armRot);
		Projectile.rotation = armRot +(player.direction == 1 ? 45f : 90f);
		SwordEnd = GetSwordTipWorld(Projectile.rotation);
	...
	private void Attack_Draw(SpriteBatch sb) {
		...
		for (int i = 0; i < oldPos.Length; i++) {
			if (oldPos[i] != Vector2.Zero) {
				...
				tail.Add(new Vertex(oldHandpos[i] - Main.screenPosition, new Vector3(progress, 0, 1),
					new Color(74, 74, 74, 255)));
				tail.Add(new Vertex(oldPos[i] - Main.screenPosition, new Vector3(progress, 1, 1),
					new Color(74, 74, 74, 255)));
			}
		}
		...
		Main.graphics.GraphicsDevice.Textures[0] = ModContent
			.Request<Texture2D>("ArknightsMod/Content/Projectiles/Defender/Durnar/Extra_209").Value;
		if (tail.Count >= 3)
			Main.graphics.GraphicsDevice.DrawUserPrimitives(PrimitiveType.TriangleStrip, tail.ToArray(), 0,
				tail.Count - 2);
		...
		DrawSwordBody(Projectile.rotation, Color.White);
