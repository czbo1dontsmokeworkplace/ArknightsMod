...
namespace ArknightsMod.Content.Items.Armor.Reforge
{
	...
	// SetProfile 的时装自动 new 一个这个类的实例注册进游戏（见 ReforgeSetLoader），
	...
	// ReforgeSetLoader 手动创建并 AddContent，类本身绝不能被自动加载。
	[Autoload(false)]
	public sealed class ReforgeSetPiece : ModItem
	{
		...
		private ReforgeVanityItem _vanity;
		private EquipType? _slotType;

		public ReforgeVanityItem Vanity => _vanity ??= ReforgeSetLoader.GetVanity(Type);
		public EquipType SlotType => _slotType ??= ReforgeSetLoader.GetSlotType(Type);
		public ReforgeSetProfile Profile => Vanity.SetProfile;

		/// <summary>外观形态开关，与对应时装各自独立记录，互不影响。</summary>
		public bool HelmetForm;

		public ReforgeSetPiece() { }

		// 只给 ReforgeSetLoader 用：创建"母版"实例后、AddContent 注册之前先把
		// Vanity/SlotType 填上——Name/Texture 这些注册阶段就要用到的属性依赖它们，
		// 而这时 this.Type 还没分配（ModType 是先 Load() 再 Register()），懒加载
		// 查表那条路还查不到，所以母版必须直接赋值。
		internal void Init(ReforgeVanityItem vanity, EquipType slotType) {
			_vanity = vanity;
			_slotType = slotType;
		}
		...
		public override void Load() {
			...
			// 本身（那是图标）——见 ReforgeAppearance.EquipTextureSuffix 的说明。
			int slot = EquipLoader.AddEquipTexture(Mod, Vanity.Texture + ReforgeAppearance.EquipTextureSuffix(SlotType), SlotType, this, Name);
			ReforgeAppearance.RecordDefaultSlot(Name, slot);
			ReforgeAppearance.HideVanillaSkin(SlotType, slot);
			ReforgeAppearance.RegisterAltEquip(Mod, SlotType, Name, Vanity.AltEquipTexture);
		}
		...
		public override void SetDefaults() {
			...
			Item.maxStack = 1;
			Item.vanity = false;
			Item.rare = ReforgeRarity.Get(Vanity.Rarity);
			...
			ReforgeAppearance.ApplyEquipSlot(Item, SlotType, Name, HelmetForm);
		...
		public override bool PreDrawInInventory(SpriteBatch spriteBatch, Vector2 position, Rectangle frame, Color drawColor, Color itemColor, Vector2 origin, float scale) {
			return ReforgeAppearance.DrawInventoryIcon(spriteBatch, position, drawColor, scale, HelmetForm, Vanity.AltIconTexture);
		}

		// 只重写 UpdateEquip、故意不重写 UpdateVanity：这样套装件放进时装/社交栏时
		// 只有外观、不给防御也不触发任何套装效果（原版对社交栏只调 UpdateVanity），
		// 满足"套装效果只在装备栏生效"的要求。
		// 装备槽位的同步由 ReforgeAppearancePlayer 每帧统一处理，这里不再重复。
		public override void UpdateEquip(Player player) {
			...
			if (!ReforgeSetLoader.IsFullSetEquipped(player, Vanity))
				return;
		...
		// 光拿着时装就能看到套装/头盔效果是什么。
		public static void AppendUpgradeHint(Mod mod, List<TooltipLine> tooltips, ReforgeSetProfile profile) {
			string prefix = profile.LocalizationPrefix;
