...
{
    public class SuspendedSilkPuppet : ModItem
    {
        ...
        {
            player.GetModPlayer<SuspendedSilkPuppetPlayer>().active = true;
        }
    }

    public class SuspendedSilkPuppetPlayer : ModPlayer
    {
        public bool active;
        private int timer;
        ...
        private void ApplyDebuffDamage()
        {
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                if (npc.active && npc.life > 0 && !npc.friendly && npc.damage > 0)
                {
                    ...
                    if (debuffCount > 0)
                    {
                        int magicDamage = 70 * debuffCount;
                        
                        IEntitySource source = Player.GetSource_Misc("SuspendedSilkPuppet");
