				...
				if (mp.Skill == 0) {
					...
					if (mp.StockCount > 0 && !mp.SkillActive) {
						for (int i = 0; i < 30; i++) {//Circle
							...
							var circle_dust = Dust.NewDustPerfect(player.MountedCenter + offset, 235, player.velocity, 200, default, 0.5f);
					...
					if (mp.SkillActive) {
						...
						if (mp.SkillTimer == 1) {
							Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), player.Center.X - 50, player.Center.Y - 100, 0, 0, ProjectileType<ChenSword_S3Lung>(), 0, 0, player.whoAmI, 0f);
						}
						...
						if (mp.SkillTimer > 10 && mp.SkillTimer <= activeTime && mp.SkillTimer % 5 == 0) {
							Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), player.Center.X, player.Center.Y, 0, 0, ProjectileType<ChenSword_S3Projectile>(), player.GetWeaponDamage(Item) * 3, 2.5f, player.whoAmI, 0f);
