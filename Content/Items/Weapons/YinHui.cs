...
{
    public class YinHui : UpgradeWeaponBase
	{
		...
		public override void HoldItem(Player player) {
			var modPlayer = Main.LocalPlayer.GetModPlayer<WeaponPlayer>();
			if (Main.myPlayer == player.whoAmI) {
				if (modPlayer.Skill == 1 && modPlayer.SkillActive && Item.type == ModContent.ItemType<YinHui>()) {
					player.GetModPlayer<yinhui2player>().yinhui2 = true;
					skill2cd++;
				}
				// S1
				...
				if (modPlayer.Skill == 2 && modPlayer.SkillActive && Item.type == ModContent.ItemType<YinHui>()) {
					player.GetModPlayer<yinhui3player>().yinhui3 = true;
				}
				if (modPlayer.Skill == 2 && !modPlayer.SkillActive) {
    ...
    {
        public override string Texture => "ArknightsMod/Content/Items/Weapons/YinHui2";
        public override void SetDefaults()
        {
    ...
    public class yinhdsz6 : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            {            
                ...
                Main.graphics.GraphicsDevice.BlendState = BlendState.AlphaBlend;
                strip2.DrawTrail();
                Texture2D 贴图1 = ModContent.Request<Texture2D>("ArknightsMod/Content/Items/Weapons/YinHui3").Value;
                Main.spriteBatch.End();
                Main.spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, Main.DefaultSamplerState, DepthStencilState.None, Main.Rasterizer, null, Main.GameViewMatrix.TransformationMatrix);
