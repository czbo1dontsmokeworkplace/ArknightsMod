...
namespace ArknightsMod.Content.NPCs.Enemy.ThroughChapter4
{
	public class Drone : ModNPC
	{
		...
		private void UpdateMovement(float targetY, bool idleMode) {
			...
			if (!idleMode) {
				bool solidAhead = IsSolidAhead(NPC.direction, 12);
				bool hitWall = (!NPC.noTileCollide && NPC.collideX) || solidAhead;
				if (turnCooldown == 0 && hitWall) {
					NPC.direction *= -1;
			...
			// —— 屏幕边缘兜底（复刻 DroneII 的写法，可按需改成 homeAnchor±半径）——
			float screenLeft = Main.screenPosition.X + Main.screenWidth / 6f;
			float screenRight = Main.screenPosition.X + Main.screenWidth * 5f / 6f - NPC.width;
		...
		private void UpdateAttackSystem(Player player) {
			...
			if (++shootTimer >= 300 && NPC.position.Y <= targetHeight) {
				Vector2 v = Vector2.Normalize(player.Center - NPC.Center) * 12f; // Drone：12f
				FirePrimary(v);
				shootTimer = 0;
			}
		}
		...
		private void FirePrimary(Vector2 velocity) {
			...
			NPC.velocity *= 0.2f;
			NPC.ai[0] = 30; // 冷却区间（Drone 无 22/14 连发）
		}
