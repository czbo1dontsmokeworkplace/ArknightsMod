...
namespace ArknightsMod.Content.NPCs.Enemy.Evolution
{
	public class SpiderMom : ModNPC
	{
		...
		private int AttackDamage = 30; // 攻击伤害
		private int Attackrange = 500; // 攻击范围
		...
		private int MoveEndFrame = 11;//移动结束帧
		private int AttackStartFrame = 12;//攻击开始帧
		private int AttackEndFrame = 22;//攻击结束帧哦~
		private int DeathStartFrame = 23;//死亡开始帧
		private int DeathEndFrame = 23;//死亡结束帧
		...
		public override void AI() {
			...
			if (!Main.player[NPC.target].dead) {
				...
				if (Inattack) {
					...
					NPC.damage = 0;
					if (attackCD == 21) {
						Projectile.NewProjectile(NPC.GetSource_FromThis(), new Vector2(NPC.Center.X, NPC.Center.Y), new Vector2((p.Center.X-NPC.Center.X)/30, (p.Center.Y- (Math.Abs(p.Center.X - NPC.Center.X)/20) - NPC.Center.Y)/30), ModContent.ProjectileType<Bloodyborn>(), AttackDamage / 2, 0.8f);
					}
		...
		public class Bloodyborn : ModProjectile {
			...
			NPC.NewNPC(Projectile.GetSource_FromThis(), (int)Projectile.Center.X, (int)Projectile.Center.Y, ModContent.NPCType<tumor2>(), 0, 0, 0, 0, 0); 
			}
			public override void AI() {
			for (int i = 0; i < 5; i++) {  // 总粒子数
				Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.Blood, Projectile.velocity.X * 0.2f, Projectile.velocity.Y * 0.2f, 100, default, 1f);
			}
				if (Projectile.timeLeft > 30) {
					Projectile.velocity.Y += 0.05f; 
			}
				if (Projectile.timeLeft > 0 && Projectile.timeLeft<30) {
			...
			public override void OnHitPlayer(Player target, Player.HurtInfo info) {
				Projectile.Kill(); // 击中玩家后销毁
		}

		public override void ModifyHitPlayer(Player target, ref Player.HurtModifiers modifiers) {
				target.GetModPlayer<Common.Players.ImmunePlayer>().ImmuneMultiplier = 0.2f; // 免疫倍数
			}
		}
}
