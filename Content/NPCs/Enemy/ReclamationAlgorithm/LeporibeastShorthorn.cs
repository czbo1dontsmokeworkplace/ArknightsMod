...
namespace ArknightsMod.Content.NPCs.Enemy.ReclamationAlgorithm
{
	...
	public class LeporibeastShorthorn : ModNPC
	{
		...
		public override float SpawnChance(NPC.Spawner spawner) {
			if (spawner.Player.ZoneForest&&!spawner.Player.ZoneCorrupt&& !spawner.Player.ZoneHallow && !spawner.Player.ZoneCrimson) {
				return SpawnCondition.OverworldDay.Chance * 0.1f;
			}
			if (spawner.Player.ZoneDesert&&!spawner.Player.ZoneCorrupt&& !spawner.Player.ZoneHallow && !spawner.Player.ZoneCrimson) {
				return SpawnCondition.OverworldDay.Chance * 0.1f; 
			}
			return 0f;
		}
