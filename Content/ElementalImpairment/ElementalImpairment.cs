...
namespace ArknightsMod.Content.ElementalImpairment.Effect
{
	...
	public class AfflictionContainer
	{
		...
		public void Update() {
			...
			// 每秒减少一次全局冷却计时（可根据需要调整调用频率，此处跟随 PostAI 每帧一次）
			if (globalCooldownTimer > 0)
				globalCooldownTimer--;

			UpdateSuppression();

			// 遍历更新所有损伤（被抑制的会直接返回 UpdateResult.None）
			foreach (var aff in Afflictions) {
				...
				if (result == UpdateResult.Burst) {
					...
					// 设置全局冷却，禁止这一段时间内施加任何损伤
					globalCooldownTimer = aff.CooldownTicks;

					// 清空其他所有损伤的积累值，并重置为 Idle（不进入冷却）
					foreach (var otherAff in Afflictions) {
						if (otherAff != aff)
							otherAff.ClearAccumulation(); // ClearAccumulation 内已将 State 设为 Idle
					}
