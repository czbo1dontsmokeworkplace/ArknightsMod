...
namespace ArknightsMod.Common.Graphics
{
	internal static class PramanixTrailMath
	{
		...
		internal static Vector3 ToVector3(this Vector2 vector) => new(vector.X, vector.Y, 0f);
	}
	...
	public class Trail : IDisposable
	{
		...
		public Trail(GraphicsDevice device, int maxPointCount, ITrailTip tip, TrailWidthFunction trailWidthFunction, TrailColorFunction trailColorFunction)
		{
			...
			ModContent.GetInstance<PramanixTrailManager>().managed.Add(this);
		}
		...
		private void GenerateMesh(out VertexPositionColorTexture[] vertices, out short[] indices, out int nextAvailableIndex)
		{
			...
			for (int k = 0; k < Positions.Length; k++)
			{
				...
				Vector2 normalToNext = PramanixTrailMath.SafeNormalize(next - current);
		...
		private void SetupMeshes()
		{
			...
			Vector2 toNext = PramanixTrailMath.SafeNormalize(NextPosition - Positions[^1]);
	...
	public class PramanixTrailManager : ModSystem
	{
